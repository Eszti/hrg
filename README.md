...
```bash
python steps/predict/predict.py -d $DATA_DIR -c config/predict_100.json 
```

Once all sentences are predicted, we [merge](steps/predict/merge.py) them into one json per model.

```bash
python steps/predict/merge.py -d $DATA_DIR -c config/merge_100.json
```
...
```bash
# Hrg - 100
python pipeline/pipeline.py -d $DATA_DIR -c config/pipeline_dev_100.json
# Hrg - 100 - argument permutation
python pipeline/pipeline.py -d $DATA_DIR -c config/pipeline_dev_100_ap.json

# Hrg - 200
python pipeline/pipeline.py -d $DATA_DIR -c config/pipeline_dev_200.json
# Hrg - 200 - argument permutation
python pipeline/pipeline.py -d $DATA_DIR -c config/pipeline_dev_200_ap.json

# Hrg - 300
python pipeline/pipeline.py -d $DATA_DIR -c config/pipeline_dev_300.json
# Hrg - 300 - argument permutation
python pipeline/pipeline.py -d $DATA_DIR -c config/pipeline_dev_300_ap.json
```
