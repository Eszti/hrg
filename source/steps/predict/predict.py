...
from source.common.triplet.wire_extraction import get_wire_extraction
...
class Predict(LoopOnSenDirs):
    ...
    def __init__(self, config=None):
        super().__init__(
            description="Script to create wire jsons from predicted bolinas labels.",
            script_name="predict",
            config=config,
        )
    ...
    def _do_for_sen(self, sen_idx, sen_dir):
        ...
        for chart_filter in self.chart_filters:
            ...
            labels_file = (
                f"{sen_dir}/bolinas/{chart_filter}/sen{sen_idx}_predicted_labels.txt"
            )
            bolinas_extractions = []
            ...
            for i, labels_line in enumerate(bolinas_extractions):
                ...
                for pp in self.postprocess:
                    ...
                    wire_extractions[pp][sen_text].append(
                        get_wire_extraction(
                            extracted_labels,
                            sen_text,
                            sen_id=int(sen_idx),
    ...
    def __save_predicted_conll(self, orig_conll, extracted_labels, extracted_conll):
        ...
        for line in lines:
            ...
            fields = line.split("\t")
            output.append(
                fields[:2] + [predicate, extracted_labels.get(fields[0], "O"), str(1.0)]
            )
