...
from source.common.report import save_bar_diagram
...
class KStat(LoopOnModels):
    def __init__(self, config=None):
        ...
        self.k_stat_dir = self._get_subdir(
            f"{self.config_name}", parent_dir=self._get_subdir("stat")
        )
        self.sen_ids = []
        self.k_values = dict()
    ...
    def _before_loop(self):
        ...
        for sen, extractions in self.gold.items():
            self.sen_ids.append(
                int(sorted(extractions, key=lambda x: int(x["sen_id"]))[0]["sen_id"])
            )
    ...
    def _do_for_model(self, model):
        for chart_filter in sorted(model["bolinas_chart_filters"]):
            for pp in sorted(model["postprocess"]):
                ...
                all_json = self.__get_all_json(
                    f"{self.in_dir}/{model_name}", chart_filter, pp
                )
    ...
    def __save_k_differences(self, k_values_df):
        k_differences = (
            k_values_df.iloc[:, -len(self.models) :]
