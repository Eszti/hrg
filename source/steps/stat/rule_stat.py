...
class RuleStat(LoopOnModels):
    ...
    def _do_for_model(self, model):
        rules = self.__get_rules_from_grammar_file(
            f"{self.grammar_dir}/{model['grammar_file']}"
        )
        stat_dict = {
            bolinas_filter: [0] * len(rules)
        ...
        df_sum_abs.to_csv(
            f"{self.rule_stat_dir}/{model['in_dir']}_sum_abs.tsv", sep="\t"
        )
        df_sum_rel.to_csv(
            f"{self.rule_stat_dir}/{model['in_dir']}_sum_rel.tsv", sep="\t"
        )

        corr_lines = []
        for name, group in df_abs.groupby("lhs"):
            group.to_csv(
                f"{self.rule_stat_dir}/{model['in_dir']}_abs_{name}.tsv", sep="\t"
            )
            df_lhs = (
                group.iloc[:, -len(stat_dict) :]
                .apply(lambda x: x * 100 / x.sum())
                .map("{:.4}".format)
            )
            self.__add_rule_cols(
                df_lhs,
                group["rule"].to_list(),
                group["lhs"].to_list(),
                group["weight"].to_list(),
            )
            df_lhs.to_csv(
                f"{self.rule_stat_dir}/{model['in_dir']}_rel_{name}.tsv", sep="\t"
            )
