...
from source.steps.bolinas.common.exceptions import GrammarError, ParserError
...
class Grammar(dict):
    ...
    @classmethod
    def load_from_file(
        cls, in_file, rule_class=VoRule, reverse=False, nodelabels=False, logprob=False
    ):
        """
            ...
            if l:
                ...
                if ";" in content:
                    ...
                    try:
                        ...
                        weight = (
                            ...
                            if not weights
                                ...
                                float(weights) if logprob else math.log(float(weights))
                            )
                        )
                    except:
                        raise GrammarError(
                            "Line %i, Rule %i: Error near end of line."
                    ...
                    try:
                        ...
                        raise GrammarError(
                            "Line %i, Rule %i: Invalid rule format."
                            % (line_count, rule_count)
                        )
                    lhs = lhs.strip()
                    if rule_count == 1:
                    ...
                    if "|" in rhsstring:
                        if not is_synchronous and rule_count > 1:
                            raise GrammarError(
                                "Line %i, Rule %i: All or none of the rules need to have two RHSs."
                                % (line_count, rule_count)
                            )
                        is_synchronous = True
                        try:
                            ...
                            raise GrammarError(
                                "Only up to two RHSs are allowed in grammar file."
                            )
                    else:
                    ...
                    try:  # If the first graph in the file cannot be parsed, assume it's a string
                        ...
                        if rhs1_type == GRAPH_FORMAT:
                            raise ParserError(
                                "Line %i, Rule %i: Could not parse graph description: %s"
                            ...
                            r1_nts = set(
                                [(ntlabel.label, ntlabel.index) for ntlabel in nts]
                            )
                    ...
                    if is_synchronous:
                        ...
                        if not r1_nts == r2_nts:
                            raise GrammarError(
                                "Line %i, Rule %i: Nonterminals do not match between RHSs: %s %s"
                    ...
                    try:
                        ...
                        raise GrammarError(
                            "Line %i, Rule %i: Could not initialize rule. %s"
                            % (line_count, rule_count, e)
                        )
                    buf = StringIO()
                    rule_count += 1
    ...
    def _compute_reachability_table_lookup(self):
        ...
        for r in self:
            ...
            if self.rhs1_type is GRAPH_FORMAT:
                ...
                terminals, nonterminals = rule.rhs1.get_terminals_and_nonterminals(
                    self.nodelabels
                )
    ...
    def terminal_filter(self, input1, input2):
        ...
        if self.rhs1_type is GRAPH_FORMAT:
            input1_terminals, input1_nts = input1.get_terminals_and_nonterminals(
                self.nodelabels
            )
        elif self.rhs1_type is STRING_FORMAT:
            input1_terminals, input1_nonterminals = _terminals_and_nts_from_string(
                input1
            )

        if input2:
            if self.rhs2_type is GRAPH_FORMAT:
                input2_terminals, input2_nts = input2.get_terminals_and_nonterminals(
                    self.nodelabels
                )
        ...
        for r in self:
            ...
            if self.rhs1_type is GRAPH_FORMAT:
                terminals1, nonterminals = self[r].rhs1.get_terminals_and_nonterminals(
                    self.nodelabels
                )
            elif self.rhs1_type is STRING_FORMAT:
            ...
            if input2:
                if self.rhs2_type is GRAPH_FORMAT:
                    terminals2, nonterminals = self[
                        r
                    ].rhs2.get_terminals_and_nonterminals(self.nodelabels)
                elif self.rhs2_type is STRING_FORMAT:
                    terminals2, nonterminals = _terminals_and_nts_from_string(
                        self[r].rhs2
                    )
