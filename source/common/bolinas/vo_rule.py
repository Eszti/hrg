...
from source.common.bolinas.rule import Rule
...
class VoRule(Rule):
    ...
    def __init__(
        ...
        if isinstance(rhs1, Hgraph):
            ...
            self.rhs1_visit_order = (
                rhs1_visit_order
                if rhs1_visit_order is not None
                else range(len(rhs1.triples(nodelabels=nodelabels)))
            )
        else:
            self.is_terminal = not any([t for t in rhs1 if type(t) is NonterminalLabel])
            self.rhs1_visit_order = (
                rhs1_visit_order if rhs1_visit_order is not None else range(len(rhs1))
            )
        ...
        if self.rhs2 is not None:
            if isinstance(rhs2, Hgraph):
                self.rhs2_visit_order = (
                    rhs2_visit_order
                    if rhs2_visit_order is not None
                    else range(len(rhs2.triples(nodelabels=nodelabels)))
                )
            else:
                self.rhs2_visit_order = (
                    rhs2_visit_order
