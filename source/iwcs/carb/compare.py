...
if __name__ == "__main__":
    ...
    for i in old_scores.keys():
        ...
        if (
            old_scores[i][0][3] + gap < old_scores[i][1][3]
            and new_scores[i][0][3] > new_scores[i][1][3] + gap
        ):
            sys1_better_examples.append(i)

        # when new eval says sys1 is better and old eval says sys2 is better
        if (
            new_scores[i][0][3] + gap < new_scores[i][1][3]
            and old_scores[i][0][3] > old_scores[i][1][3] + gap
        ):
