    ...
    def compare(
        ...
        for sent, goldExtractions in gold.items():
            ...
            for conf in sent_confidences:
                ...
                if strategy == "sm":
                    ...
                    for i, row in enumerate(scores):
                        max_recall_row = max(
                            [row[ext_indx][1] for ext_indx in ext_indices], default=0
                        )
                ...
                for t in range(num_precision_matches):
                    ...
                    matched_precision = (
                        -1
                    )  # initialised to <0 so that it updates whenever precision is 0 as well
                    for i in range(len(scores)):
                        if i in selected_rows:
                            continue
                        for ext_indx in ext_indices:
                ...
                if strategy == "ss":
                    ...
                    for t in range(num_recall_matches):
                        ...
                        matched_recall = (
                            -1
                        )  # initialised to <0 so that it updates whenever recall is 0 as well
                        for i in range(len(scores)):
                            if i in selected_rows:
                                continue
                            for ext_indx in ext_indices:
        ...
        with open(output_fn, "w") as fout:
            ...
            for cur_p, cur_r, cur_conf in sorted(
                zip(prec_scores, rec_scores, confidence_thresholds),
        ...
        if len(f1s) > 0:
            return (
                np.round(auc(temp_rec_scores, temp_prec_scores), 4),
                optimal,
                zero_conf_point,
            )
...
if __name__ == "__main__":
    ...
    print(
        "AUC: {}\t Optimal (precision, recall, F1): {}\tZero Conf (precision, recall, F1): {}".format(
            auc, optimal_f1_point[:3], zero_conf_point[:3]
        )
    )
