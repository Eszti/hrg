...
class Matcher:
    ...
    def bleuMatch(ref, ex, ignoreStopwords, ignoreCase):
        ref_arg1, ref_pred, ref_arg2 = (
            ref.args[0].strip(),
            ref.pred.strip(),
            " ".join(ref.args[1:]).strip(),
        )
        ref_ext = (
            "(subject) "
            + ref_arg1
        ...
        ex_arg1, ex_pred, ex_arg2 = (
            ex.args[0].strip(),
            ex.pred.strip(),
            " ".join(ex.args[1:]).strip(),
        )
        ex_ext = (
            "(subject) " + ex_arg1 + " (relation) " + ex_pred + " (object) " + ex_arg2
        )
        prec_bleu = sentence_bleu(
            references=[ref_ext.split(" ")], hypothesis=ex_ext.split(" ")
        )
        recall_bleu = sentence_bleu(
            references=[ex_ext.split(" ")], hypothesis=ref_ext.split(" ")
        )
    ...
    def bert_score_match(ref, ex, ignoreStopwords, ignoreCase):
        global scorer
        if scorer == None:
        ...
        prec, rec, f1 = scorer.score([ex_ext], [ref_ext])
        # prec, rec, f1 = scorer.score([ex_arg1, ex_pred, ex_arg2], [ref_arg1, ref_pred, ref_arg2])
        prec, rec = (prec + 1) / 2, (rec + 1) / 2
        prec, rec = prec.mean(), rec.mean()
    ...
    def binary_linient_tuple_match(ref, ex, ignoreStopwords, ignoreCase):
        ...
        if not said_type_sentence:
            ...
            if len(ex.args) >= 2:
                ...
                e = ex
            reverse_match = Matcher.linient_tuple_match(
                r, e, ignoreStopwords, ignoreCase
            )
