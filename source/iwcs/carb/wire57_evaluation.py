...
def get_allenlp_args(line):
    ...
    arg1 = (
        re.findall("<arg1>.*</arg1>", line)[0].strip("<arg1>").strip("</arg1>").strip()
    )
    rel = re.findall("<rel>.*</rel>", line)[0].strip("<rel>").strip("</rel>").strip()
    arg2 = (
        re.findall("<arg2>.*</arg2>", line)[0].strip("<arg2>").strip("</arg2>").strip()
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gold", help="file path for gold in allennlp format", required=True
    )
    parser.add_argument(
        "--system", help="file path for system in allennlp format", required=True
    )
