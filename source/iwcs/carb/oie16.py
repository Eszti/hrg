...
class Benchmark:
    ...
    def compare(
        self, predicted, matchingFunc, output_fn, error_file=None, pickle_output_fp=None
    ):
        ...
        for sent, item in gold.items():
            ...
            predictedExtractions, predictedSent = (
                predicted[sent]["extractions"],
                predicted[sent]["orig_sent"],
            )
            for goldEx in goldExtractions:
                correctTotal += 1
                found = False
                ...
                for predictedEx in predictedExtractions:
                    ...
                    if matchingFunc(
                        goldEx, predictedEx, ignoreStopwords=True, ignoreCase=True
                    ):
                        # print('***Matching***')
            ...
            for predictedEx in [
                x for x in predictedExtractions if (output_fn not in x.matched)
            ]:
                # Add false positives
            ...
            if type(pickle_output) != type(None):
                ...
                ), optimal = Benchmark.prCurve(
                    sentence_y_true.copy(),
                    sentence_y_scores.copy(),
                    recallMultiplier=(
                        (len(goldExtractions) - sentence_unmatchedCount + 1)
                        / float(len(goldExtractions))
                    ),
                    sentence_level=True,
                )
                ...
                if len(sentence_precision) < num_predictions:
                    ...
                    for indx in range(num_predictions):
                        conf_indx = np.where(
                            confidence_thresholds == sorted_y_scores[indx]
                        )[0]
                ...
                predictedExtractions = sorted(
                    predictedExtractions, key=lambda x: x.confidence, reverse=True
                )
                ...
                pickle_output[goldSent] = {
                    ...
                    "zero_conf_score": (
                        round(zero_conf_prec, 4),
                        round(zero_conf_recall, 4),
                    ),
                }
        ...
        if error_file:
            logging.info(
                "Writing {} error indices to {}".format(len(errors), error_file)
            )
    ...
    def prCurve(y_true, y_scores, recallMultiplier, sentence_level=False):
        ...
        y_scores = [
            score if not (np.isnan(score) or (not np.isfinite(score))) else 0
        ...
        optimal = max(
            [
                ...
                for ((precision, recall), threshold) in zip(
                    zip(precision_ls[:-1], recall_ls[:-1]), thresholds
                )
            ],
            key=itemgetter(2),
        )  # Sort by f1 score
    ...
    def normalizeDict(d):
        return dict(
            [
                (Benchmark.normalizeKey(k).lower(), {"orig_sent": k, "extractions": v})
                for k, v in d.items()
            ]
        )
